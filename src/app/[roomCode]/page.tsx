...
export default function RoomAdminPage() {
  ...
  async function handlePrev() {
    ...
    if (!playing) return
    await fetch(`/api/queue/${playing.id}?room=${roomCode}`, {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
    ...
    <div className="min-h-screen bg-gray-950 text-white flex flex-col">
      {showShortcuts && (
        ...
        >
          ...
          >
            <h3 className="text-lg font-bold mb-4 flex items-center gap-2">⌨️ คีย์ลัด</h3>
      ...
      <header className="px-6 py-4 border-b border-gray-800 flex items-center gap-3">
        ...
        <span className="text-xs text-gray-600 font-mono border border-gray-700 rounded px-1.5 py-0.5">{roomCode}</span>
        {currentSong && (
      ...
      <div className="px-6 py-3 border-b border-gray-800 bg-gray-900/50">
        <AddSongForm roomCode={roomCode} onAdded={fetchQueue} />
      </div>
